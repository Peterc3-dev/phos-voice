...
echo "[1/4] Termux packages (ffmpeg, termux-api, python, wget)…"
pkg install -y ffmpeg termux-api python wget >/dev/null

echo "[2/4] copy app files + binaries -> $DIR…"
...
echo "[3/4] whisper STT model (~57 MB, one-time)…"
[ -f "$DIR/ggml-base.en-q5_1.bin" ] || wget -q --show-progress -O "$DIR/ggml-base.en-q5_1.bin" "$WURL"

echo "[4/4] LLM model — Qwen3-1.7B Q4_K_M (~1.1 GB, one-time)…"
